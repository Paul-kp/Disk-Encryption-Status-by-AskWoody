@(set "0=%~f0"^)#) & start "" powershell -nop -c $_File=$env:0; iex([io.file]::ReadAllText($env:0)) & exit /b
#:: run the bat file - it's a standalone hybrid powershell script
#######################################
...
# Version: 2.0
# Date: 20250213
# Changes: converted to a hybrid script that runs as a BAT/CMD file. Kudos to https://github.com/AveYo/fox
...
Function ElevateUser
{
	...
	$SPArgs = @{
		...
		# the script is a batch file, we need to read it into memory to execute as a script.
		argumentlist = "-nop -c iex([io.file]::ReadAllText('$_File'))"
		}
...
## check if script was loaded as a hybrid
If ((Get-AdminStatus) -eq "User" -and $_File.length -lt 5) { 
		ShowMessage "This script must be run from a BAT/CMD file`nSee the documentation" "Hybrid File Check"
		exit
}
